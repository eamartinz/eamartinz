# ARQUIVO 02
# Script para treinamento de uma CNN para classificação de boca (Yawn vs No_Yawn)
#
...
# ========================================================================
# ARQUITETURA DA CNN PARA CLASSIFICAÇÃO DE BOCA (Yawn vs No_Yawn)
# ========================================================================

class MouthClassifierCNN(nn.Module):
    """
    Rede Neural Convolucional para classificar se a boca está bocejando ou não.
    
    ARQUITETURA:
    - Entrada: Imagens RGB 64x64 pixels
    - Saída: 2 classes (No_Yawn, Yawn)
    
    Mesma estrutura da EyeClassifierCNN, otimizada para detecção de bocejo.
    """
    ...
    def __init__(self, num_classes=2):
        ...
        Args:
            num_classes (int): Número de classes (padrão: 2 para Yawn/No_Yawn)
        """
...
# Instancia a CNN
model = MouthClassifierCNN(num_classes=2)
...
print("\n" + "="*70)
print("INICIANDO TREINAMENTO DA CNN (BOCA - YAWN vs NO_YAWN)")
print("="*70 + "\n")
...
for epoch in range(NUM_EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), str(MODELS_DIR / 'mouth_model_best.pth'))
...
ax.set_title('Loss durante Treino e Validação - Boca (Yawn vs No_Yawn)', fontsize=14, fontweight='bold')
ax.legend(fontsize=11)
ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(str(MODELS_DIR / 'mouth_loss_plot.png'), dpi=300, bbox_inches='tight')
print(f"✓ Gráfico de Loss salvo: {MODELS_DIR / 'mouth_loss_plot.png'}")
plt.close()
...
ax.set_title('Acurácia durante Treino e Validação - Boca (Yawn vs No_Yawn)', fontsize=14, fontweight='bold')
ax.set_ylim([0, 105])
ax.legend(fontsize=11)
ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(str(MODELS_DIR / 'mouth_accuracy_plot.png'), dpi=300, bbox_inches='tight')
print(f"✓ Gráfico de Acurácia salvo: {MODELS_DIR / 'mouth_accuracy_plot.png'}")
plt.close()
...
ax.set_title('Matriz de Confusão - Conjunto de Teste (Boca)', fontsize=14, fontweight='bold')

plt.tight_layout()
plt.savefig(str(MODELS_DIR / 'mouth_confusion_matrix.png'), dpi=300, bbox_inches='tight')
print(f"✓ Matriz de Confusão salvo: {MODELS_DIR / 'mouth_confusion_matrix.png'}")
plt.close()
...
axes[0].set_title('Loss - Treino vs Validação (Boca)', fontsize=12, fontweight='bold')
axes[0].legend()
...
axes[1].set_title('Acurácia - Treino vs Validação (Boca)', fontsize=12, fontweight='bold')
axes[1].set_ylim([0, 105])
axes[1].legend()
axes[1].grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(str(MODELS_DIR / 'mouth_training_metrics_combined.png'), dpi=300, bbox_inches='tight')
print(f"✓ Gráfico combinado salvo: {MODELS_DIR / 'mouth_training_metrics_combined.png'}")
plt.close()
...
torch.save(model.state_dict(), str(MODELS_DIR / 'mouth_model_final.pth'))
print(f"✓ Pesos do modelo salvos: {MODELS_DIR / 'mouth_model_final.pth'}")

# Salvar modelo completo (inclui arquitetura)
torch.save(model, str(MODELS_DIR / 'mouth_model_complete.pth'))
print(f"✓ Modelo completo salvo: {MODELS_DIR / 'mouth_model_complete.pth'}")
...
torch.save(checkpoint, str(MODELS_DIR / 'mouth_model_checkpoint.pth'))
print(f"✓ Checkpoint salvo: {MODELS_DIR / 'mouth_model_checkpoint.pth'}")
...
print("\n" + "="*70)
print("RESUMO DO TREINAMENTO - BOCA (YAWN vs NO_YAWN)")
print("="*70)
...
print("  - mouth_loss_plot.png: Gráfico de Loss (Treino vs Validação)")
print("  - mouth_accuracy_plot.png: Gráfico de Acurácia (Treino vs Validação)")
print("  - mouth_confusion_matrix.png: Matriz de Confusão do Teste")
print("  - mouth_training_metrics_combined.png: Gráficos combinados")
print("  - mouth_model_final.pth: Pesos do modelo treinado")
print("  - mouth_model_complete.pth: Modelo completo")
print("  - mouth_model_checkpoint.pth: Checkpoint com metadados")
print("="*70 + "\n")
